...
class SNN(Module):
    def __init__(self, start_features): 
        ...
        self.fc2 = Linear(in_features=10, out_features=5, bias=False)
        self.fc3 = Linear(in_features=5, out_features=3, bias=False)
    ...
    def xe(self, x1, x2):
        y1 = F.relu(self.fc3(F.relu(self.fc2(F.relu(self.fc1(x1))))))      # (batchsize, 3)
        y2 = F.relu(self.fc3(F.relu(self.fc2(F.relu(self.fc1(x2))))))      # (batchsize, 3)        
        y = torch.cat([y1, y2], 1)
